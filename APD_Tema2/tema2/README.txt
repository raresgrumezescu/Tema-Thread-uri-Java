...
- Reduce (clasa pe care o folosesc sa creez obiecte folosite ca task-uri in etapa Reduce)
...
In continuare, vectorul de rezultate il prelucrez, pe baza obiectelor din el creand noile obiecte de tip Task2, si le pun
intr-un vector pe toate. Acest vector il voi folosi la etapa Reduce.

Dupa ce obtin vectorul de task-uri de tip Task2, numit taskuri2, redeschid thread-urile, de data aceasta folosind clasa MyThread2.
Fiecare thread disapune de o metoda run in care parcurg vectorul de task-uri taskuri2 la fel cum parcurgeam taskuri in MyThread.
Sar din nr_workeri in nr_workeri prin el si am grija ca fiecare thread sa isi acceseze numai bucata ce ii este lui destinata.
In interiorul acestei metode calculez rangul pentru fiecare fisier in parte (Un obiect Task2 corespunde unui fisier).

rezultatele de pe urma acestei prelucrari le stuchez in alt vector, numit arr_out, de obiecte de tip Outputuri. Aceste obiecte
